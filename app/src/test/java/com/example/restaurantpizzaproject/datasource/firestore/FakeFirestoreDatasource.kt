...
val order1 = Order("id1", "clientId","12/02/2021", "address",
    listOf(CartDetail("Mussarela", 50.0, 1)),
    50.0, "cartão", OrderStatus.OPEN
)
val order2 = Order("id1", "clientId","12/02/2021", "address",
    listOf(CartDetail("Mussarela", 50.0, 1)),
    50.0, "cartão", OrderStatus.CANCELLED
)
...
class FakeFirestoreDatasource(
    ...
    override fun getCancelledAndDeliveredOrders(): Flow<List<Order>?> = flow {
        emit(orderList.filter { it.status == OrderStatus.CANCELLED })
    }
